...
import com.example.amantoliv2.Model.User
...
class SignUpActivity : AppCompatActivity() {
    ...
    private val userCollectionRef = Firebase.firestore.collection("Users")//Colleción Users
    ...
    private fun checkInput() {
        ...
        if (!emailEt.text.matches(emailPattern.toRegex())) {
            toast("Ingrese un correo válido")
            return
        }
        if(passEt.text.isEmpty()){
            ...
            firebaseAuth.createUserWithEmailAndPassword(emailV,passV)
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        progressDialog.setMessage("Guardar datos de usuario")
